...
impl OptimizerConfig {
    ...
    pub fn validate(&self) -> OptimizerResult<()> {
        ...
        if self.optimization.artifact_retention_days > 365 {
            return Err(OptimizerError::config("Artifact retention cannot exceed 365 days"));
        }

        // Validate tool install timeout
        if self.tools.install_timeout_seconds < 30 {
            return Err(OptimizerError::config("Tool install timeout must be at least 30 seconds"));
        }
...
pub fn generate_cargo_config(config: &OptimizerConfig, system_info: &crate::system::SystemInfo) -> String {
    ...
    content.push_str(&format!("CARGO_INCREMENTAL = \"{}\"\n", if config.build.incremental { "1" } else { "0" }));
    ...
    if config.build.use_fast_linker {
        if let Some(linker) = system_info.get_recommended_linker() {
            match (&system_info.os, &system_info.arch) {
                (crate::system::OperatingSystem::MacOS, crate::system::Architecture::Aarch64) => {
                    ...
                    content.push_str(&format!("    \"-C\", \"target-cpu={}\",\n", config.build.target_cpu));
