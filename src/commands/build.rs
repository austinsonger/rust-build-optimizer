...
pub async fn run(build_type: BuildCommands, project_dir: Option<PathBuf>) -> OptimizerResult<()> {
    ...
    match build_type {
        BuildCommands::Check { stats } => {
            run_check(&project_root, stats).await
        }
        BuildCommands::Build { release, stats } => {
            run_build(&project_root, release, stats).await
        }
        BuildCommands::Test { changed, stats } => {
            run_test(&project_root, changed, stats).await
        }
        BuildCommands::Clean { all } => {
            run_clean(&project_root, all).await
        }
    }
}
...
async fn run_check(project_root: &PathBuf, show_stats: bool) -> OptimizerResult<()> {
    ...
    match result {
        Ok(_) => {
            print_success(&format!("✅ Check completed in {}", format_duration(duration)));
...
async fn run_build(project_root: &PathBuf, release: bool, show_stats: bool) -> OptimizerResult<()> {
    ...
    print_status(&format!("Running optimized cargo build ({})...", build_type));
    ...
    match result {
        Ok(_) => {
            print_success(&format!("✅ Build completed in {}", format_duration(duration)));
...
async fn run_test(project_root: &PathBuf, _changed: bool, show_stats: bool) -> OptimizerResult<()> {
    ...
    let result = if is_tool_available("cargo-nextest") {
        ...
        execute_command_with_output("cargo", &["nextest", "run", "--workspace"], Some(project_root))
    } else {
    ...
    match result {
        Ok(_) => {
            print_success(&format!("✅ Tests completed in {}", format_duration(duration)));
...
async fn show_build_stats(project_root: &PathBuf, build_type: &str, duration: std::time::Duration) -> OptimizerResult<()> {
    println!();
    ...
    if let Ok(output) = execute_command("cargo", &["tree", "--workspace", "--depth", "0"], Some(project_root)) {
